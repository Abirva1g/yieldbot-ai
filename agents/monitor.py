...
class MonitorAgent:
    ...
    def __init__(self):
        self.degraded_failure_threshold = settings.monitor.degraded_failure_threshold
        self.critical_failure_threshold = settings.monitor.critical_failure_threshold
        self.degraded_success_rate_threshold = settings.monitor.degraded_success_rate_threshold
        self.critical_success_rate_threshold = settings.monitor.critical_success_rate_threshold
        self.cooldown_minutes = settings.monitor.cooldown_minutes
    
    def calculate_success_rate(self, history: List[TradeResult], window: int) -> float:
    ...
    async def check_and_heal(self, state: BotState) -> BotState:
        ...
        if consecutive_failures >= self.critical_failure_threshold:
            new_status = HealthStatus.CRITICAL
            logger.warning(f"Monitor: Critical status - {consecutive_failures} consecutive failures")
        elif self.calculate_success_rate(execution_history, 20) < self.critical_success_rate_threshold:
            new_status = HealthStatus.CRITICAL
        ...
        elif consecutive_failures >= self.degraded_failure_threshold:
            new_status = HealthStatus.DEGRADED
            logger.warning(f"Monitor: Degraded status - {consecutive_failures} consecutive failures")
        elif self.calculate_success_rate(execution_history, 10) < self.degraded_success_rate_threshold:
            new_status = HealthStatus.DEGRADED
        ...
        if new_status == HealthStatus.CRITICAL:
            ...
            logger.error(f"Monitor: PAUSING trading for {self.cooldown_minutes} minutes")
        ...
        # Log health summary
        success_rate_10 = self.calculate_success_rate(execution_history, 10)
        success_rate_20 = self.calculate_success_rate(execution_history, 20)
        logger.info(
